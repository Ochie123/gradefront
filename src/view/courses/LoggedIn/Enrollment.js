...
import { loadEnrolleds, loadGrades, loadCourses, loadCourse } from '../../../data/api/api';


export default function Enrollment(props) {
  const { id } = useParams();

  const { data: course } = useQuery(["currentCourse", { id }], () => loadCourse(id));

  const { data: enrolledsData = { results: [] } } = useQuery("enrolleds", loadEnrolleds);
  const enrolleds = enrolledsData.results;

  const { data: coursesData = { results: [] } } = useQuery("courses", loadCourses);
  ...
  const userEnrolleds = enrolleds.filter(enrolled => enrolled.user === savedClaims?.user_id);
  //console.log(userEnrolleds);

  const enrolledCourses = courses.filter(course => {
    const isEnrolled = userEnrolleds.some(enrolled => enrolled.course === course.uuid);
    return isEnrolled;
  });

  let CourseGrades = grades.filter(grade => grade.course === course?.uuid)
